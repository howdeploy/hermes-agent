...
### Headless configuration and access

Interactive CLI chats and routed Telegram chats can use enabled local teammates
without Desktop metadata. Telegram uses `$name` as a display-safe alias; the
`message_agent` tool accepts both `$name` and `@name`.

To enable Bot Mode declaratively for canonical Bot Chats and human messaging
gateways, configure the machine-root `config.yaml`:
...
The optional roster restricts **local** recipients per sender. An omitted roster
keeps legacy unrestricted local routing; an empty or malformed roster grants no
local targets. Explicit `enabled: false` or unreadable root configuration denies
Bot Mode. Each target must also have `bot.enabled: true` in `profile.yaml`
(omitted metadata retains the legacy enabled default; corrupt/unreadable metadata
denies execution). Source checks run first: API/A2A, cron, delegated children,
finite queries, internal Bot Chain and Desktop group-room turns cannot acquire
messaging authority by supplying a trusted-looking title or restoring a cached
session. Dispatch rechecks the live policy even when the prompt was built earlier.
